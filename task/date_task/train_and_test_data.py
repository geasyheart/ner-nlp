...
class DataProcess(object):
    def __init__(self, max_len: int = 128):
        self.df = pd.read_excel(TRAIN_DATA_PATH, sheet_name="时间").fillna("")
        self.proportion()
    ...
    def to_file(self):
        ...
        for index, row in self.df.iterrows():
            ...
            if not isinstance(data, str):
                continue
            data = data.replace(" ", "")
            labels = [value for key, value in row.items() if value and key not in ("Unamed", "编号", "原文")]
        ...
        final_data_as_np = final_data_as_np
        final_labels_as_np = final_labels_as_np
