...
class SpaceDataProcess(object):
    ...
    @property
    def space_df(self):
        return pd.read_excel(TRAIN_DATA_PATH, sheet_name="空间").fillna("")
    ...
    def proportion(self):
        ...
        """
        p = {}
        for index, row in self.space_df.iterrows():
            data = str(row['原文'])
            data_len = len(data)
            p[data] = data_len
    ...
    def to_file(self):
            ...
            for label in labels:
                ...
                for _index, _label in enumerate(data):
                    if _index == start_index:
                        tmp_label.append('B-LOC')
                    elif start_index < _index < end_index:
                        tmp_label.append('I-LOC')
                    else:
                        tmp_label.append('O')
        ...
        #         with open(os.path.join(DATA_DIR, "loc_train_data.txt"), "a+") as f:
        #             newline = "\n".join([" ".join(line) for line in zip(d, l)])
        #             f.write(newline)
        #             f.write("\n\n")
        #     else:
        #         with open(os.path.join(DATA_DIR, "loc_test_data.txt"), "a+") as f:
        #             newline = "\n".join([" ".join(line) for line in zip(d, l)])
        #             f.write(newline)
        #             f.write("\n\n")
